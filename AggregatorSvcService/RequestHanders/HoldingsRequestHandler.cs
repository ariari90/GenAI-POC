    ...
    public class HoldingsRequestHandler: RequestHandler
    {
        ...
        public override AggregatorResponse ProcessData()
        {
            ...
            using (WorkflowRuntime workflowRuntime = new WorkflowRuntime())
            {
                ...
                var orchestrator = AggregatorConstants.orchestratorConfig.Orchestrators.FirstOrDefault(x => x.Type == AggregatorConstants.DPO_Holdings);
                if (orchestrator == null)
                {
                    _log.LogError("OrchestratorConfig does not contain the required key: " + AggregatorConstants.DPO_Holdings);
